...
class StreamDSL {
    ...
    countByKey(key = "key", countFieldName = "count"){
        ...
        //TODO fix forEach is ran once but map is ran twice

        this.forEach(element => {
            console.log("before " + element[key] + " for topic: " + this.topicName);
        });

        //no async or promise action
        this.map(element => {

            console.log("incrementing " + element[key] + " for topic: " + this.topicName);

            //atomic
            const k = element[key];
            this.storage.state[k] = (this.storage.state[k] || 0) + 1;

            //cloned
            const clone = JSON.parse(JSON.stringify(element));
            clone[countFieldName] = this.storage.state[k];
    ...
    to(topic, outputPartitionsCount = 1, producerErrorCallback = null){
        return new Promise(resolve => {
            ...
            this.forEach(message => this.kafka.send(this.outputTopicName, message));
        });
    }
}
